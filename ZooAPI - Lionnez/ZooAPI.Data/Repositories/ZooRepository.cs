...
namespace ZooAPI.Data.Repositories
{
    public class ZooRepository : IZooRepository
    {
        ...
        {
            var db = DbConnection();
            var sql = "INSERT INTO tbAnimal (nome, tipo_id, descricao, habitat, pais, alimentacao, peso, altura, curiosidades, imagem, imagemDois, imagemTres, imagemQuatro, tipoanim, habitatResum) " +
                      "VALUES (@Nome, @TipoId, @Descricao, @Habitat, @Pais, @Alimentacao, @Peso, @Altura, @Curiosidades, @Imagem, @ImagemDois, @ImagemTres, @ImagemQuatro, @TipoAnim, @HabitatResum)";
        ...
        public async Task<bool> UpdateAnimal(Animal animal)
        {
            ...
            var sql = "UPDATE tbAnimal SET nome = @Nome, tipo_id = @TipoId, descricao = @Descricao, habitat = @Habitat, " +
                      ...
                      "curiosidades = @Curiosidades, imagem = @Imagem, imagemDois = @ImagemDois, imagemTres = @ImagemTres," +
                      "imagemQuatro = @ImagemQuatro, tipoanim = @TipoAnim, habitatResum = @HabitatResum WHERE id = @Id";
            var result = await db.ExecuteAsync(sql, animal);
            return result > 0;
        }
